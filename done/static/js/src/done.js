...
function update_knob(element, data) {
  ...
  if($('.done_onoffswitch-checkbox', element).prop("checked")) {
    ...
    $(".done_onoffswitch-checkbox").attr('disabled','disabled');
  } else {
       ...
       var totalTime = data.time; // Amout of time (sec)
       ...
       if (!$.cookie('cdTime')) {
          ...
          $.cookie('cdTime', totalTime, {
             expires: 7,
             path: '/'
          });
          var runTimer = $.cookie('cdTime');
       } else {
          var currentTime = $.now();
          var usedTime = (currentTime - $.cookie('firstTime')) / 1000; // Calculate and convert into seconds
          var runTimer = $.cookie('cdTime') - usedTime;
       }
       $('#cd').countdown({
          until: runTimer,
          compact: true,
       ...
       function EndCountdown() {
          ...
          $('.done_onoffswitch-checkbox').removeAttr('disabled');
          $('.done_onoffswitch_wrapper').show();
           $('#cd').html('');
       }
 }
}
